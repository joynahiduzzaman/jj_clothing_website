...
const FAQS = [
  { q: "Where are your pieces made?", a: "We work directly with our manufacturing partners and check every piece for fit, fabric and finish before it ships." },
  ...
  { q: "What if something doesn't fit?", a: "Unworn products with tags attached can be returned within 7 days of delivery. Full details are on our Refund Policy page." },
];

export default async function AboutPage() {
  const [content, nav] = await Promise.all([getPageContent("about"), getNavMenu()]);
  const collectionPartners = nav.collections;
    ...
    <div>
      ...
      <section className="container-px mx-auto section-py">
        <ScrollReveal className="rounded-xl2 bg-ink text-cream p-10 md:p-14 grid md:grid-cols-[1.2fr_1fr] gap-10 items-center">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-rose-gold-light font-semibold mb-4">Quality Promise</p>
            <h2 className="font-display text-2xl md:text-3xl font-semibold mb-4">
              Every piece is checked for fit, fabric and finish before it ships — considered, not just claimed.
            </h2>
            <p className="text-cream/60 text-sm leading-relaxed max-w-md">
              We're upfront about how each piece is made and checked. See our Shipping and Refund Policy pages for exactly how delivery and returns work.
            </p>
          </div>
          <Link href="/shop" className="btn-primary w-fit md:justify-self-end">
      ...
      <section className="container-px mx-auto section-py">
        <ScrollReveal className="text-center max-w-lg mx-auto mb-14">
          <p className="text-xs uppercase tracking-[0.2em] text-gold-text font-semibold mb-4">Collections</p>
          <h2 className="section-title">Shop by collection.</h2>
        </ScrollReveal>
        {/* Read from the catalogue rather than hardcoded, so a collection wall
            never links to an empty page. */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 md:gap-6">
          {collectionPartners.map((collection, i) => (
            <ScrollReveal key={collection.href} delay={i * 0.05}>
              <Link
      ...
      <section className="container-px mx-auto pb-24">
        <ScrollReveal className="rounded-xl2 bg-rose-gold text-white text-center p-14 md:p-20">
