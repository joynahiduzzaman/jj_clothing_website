  ...
  async function handlePlaceOrder() {
    ...
    try {
      ...
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          items: items.map((i) => ({ productId: i.productId, variantId: i.variantId, name: i.name, image: i.image, price: i.price, quantity: i.quantity })),
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_360px] gap-6 md:gap-10">
          ...
          {step === 1 && (
            <section>
              ...
              <div className="grid grid-cols-2 gap-4">
                <input required aria-label={dict.checkout.fullName} placeholder={dict.checkout.fullName} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="field col-span-2" />
                ...
                <input
                  type="email"
                  aria-label={dict.checkout.email}
                ...
                <textarea required aria-label={dict.checkout.street} placeholder={dict.checkout.street} value={form.street} onChange={(e) => setForm({ ...form, street: e.target.value })} className="field col-span-2" rows={2} />
                ...
                <input aria-label={dict.checkout.giftNote} placeholder={dict.checkout.giftNote} value={form.giftNote} onChange={(e) => setForm({ ...form, giftNote: e.target.value })} className="field col-span-2" />
              </div>
        ...
        <aside className="bg-beige/50 rounded-xl2 p-6 h-fit space-y-4">
              ...
              <div key={`${item.productId}-${item.variantId}`} className="flex gap-3 text-sm">
                ...
                <div className="flex-1">
                  ...
                  {(item.color || item.size) && (
                    <p className="text-ink/50 text-xs">{[item.color, item.size].filter(Boolean).join(" / ")}</p>
                  )}
          ...
          <div className="flex gap-2">
            <input value={couponCode} onChange={(e) => setCouponCode(e.target.value)} aria-label={dict.cart.coupon} placeholder={dict.cart.coupon} className="flex-1 rounded-full px-4 py-2 text-sm border border-ink/10" />
