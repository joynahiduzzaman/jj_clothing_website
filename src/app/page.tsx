...
const getAllCollections = unstable_cache(
  async () => {
    // Every collection now has a logo uploaded, so the tile leads with the
    // mark. One in-stock product image is still pulled per collection as the
    // fallback visual, so a newly added collection never renders as an empty
    // tile before its logo arrives — that is what these tiles showed back
    // when no logos existed.
    ...
    return collections.map(({ products, _count, ...collection }) => ({
      ...collection,
      ...
      const collections =
        settings.mode === "manual" && Array.isArray(settings.brandIds) && settings.brandIds.length > 0
          ? settings.brandIds
              ...
              .filter((c: typeof all[number] | undefined): c is typeof all[number] => Boolean(c))
              .slice(0, limit)
          : // Stocked collections first, same rule the category wall follows —
            // a homepage tile that leads to an empty shelf is a dead end, and
            // several collections are set up ahead of their first drop.
            [...all].sort((a, b) => Number(b.productCount > 0) - Number(a.productCount > 0)).slice(0, limit);
      return <FeaturedCollections collections={collections} title={settings.title} subtitle={settings.subtitle} backgroundColor={bg} />;
    }
    case "newArrivals": {
      ...
      return products.length > 0 ? (
        <ProductRail
          ...
          eyebrow="Just Landed"
          subtitle={settings.subtitle}
          description="Fresh in, before it sells through."
