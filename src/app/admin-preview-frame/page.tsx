...
export default function PreviewFramePage() {
  ...
  useEffect(() => {
    function onMessage(e: MessageEvent) {
      ...
      if (e.data?.type !== "admin-preview-update") return;
      setSectionKey(e.data.sectionKey);
      setSettings(e.data.settings || {});
    }
    window.addEventListener("message", onMessage);
    window.parent.postMessage({ type: "admin-preview-ready" }, window.location.origin);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      window.parent.postMessage({ type: "admin-preview-height", height: document.documentElement.scrollHeight }, window.location.origin);
    });
  ...
  function renderContent() {
    ...
    switch (sectionKey) {
      ...
      case "brandShowcase":
        return <FeaturedCollections collections={data?.collections || []} title={settings.title} subtitle={settings.subtitle} backgroundColor={bg} />;
      case "newArrivals":
        return (
          <ProductRail
