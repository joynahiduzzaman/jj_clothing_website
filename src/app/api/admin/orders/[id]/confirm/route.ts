...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const missingVariant = order.items.find((i) => !i.variantId);
    if (missingVariant) {
      return NextResponse.json({ error: `${missingVariant.name} has no variant on record and can't be confirmed` }, { status: 400 });
    }
    const { verifiedItems } = await verifyOrderItems(
      order.items.map((i) => ({ productId: i.productId, variantId: i.variantId as string, quantity: i.quantity })),
