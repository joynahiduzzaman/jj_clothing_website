...
const schema = z.object({
  ...
  action: z.enum(["delete", "activate", "draft", "changeCategory", "changeCollection"]),
  value: z.string().optional(), // categoryId or collectionId, required for those two actions
});
...
export async function POST(req: NextRequest) {
  ...
  if (action === "changeCollection") {
    if (!value) return NextResponse.json({ error: "Select a collection" }, { status: 400 });
    await prisma.product.updateMany({ where: { id: { in: ids } }, data: { collectionId: value } });
    return NextResponse.json({ success: true, count: ids.length });
  }
