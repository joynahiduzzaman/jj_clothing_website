...
export async function GET(req: NextRequest) {
  ...
  if (type === "collections") {
    // Mirrors getAllCollections() on the homepage — includes a representative
    // product image so the builder preview looks like the real section, not a
    // fallback.
    ...
    const all = raw.map(({ products, ...collection }) => ({
      ...collection,
