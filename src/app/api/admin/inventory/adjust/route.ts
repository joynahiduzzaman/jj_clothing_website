...
export async function POST(req: NextRequest) {
  ...
  const newStock = await adjustStock(parsed.data.variantId, parsed.data.change, parsed.data.reason, user.name);
  if (newStock === null) return NextResponse.json({ error: "Variant not found" }, { status: 404 });
