...
export async function POST(req: NextRequest) {
  ...
  const order = await prisma.order.findUnique({
    ...
    include: {
      items: { select: { id: true, name: true, color: true, size: true, image: true, price: true, quantity: true } },
  ...
  return NextResponse.json({
    order: {
      ...
      items: order.items.map((i) => ({ name: i.name, color: i.color, size: i.size, image: i.image, price: i.price, quantity: i.quantity })),
