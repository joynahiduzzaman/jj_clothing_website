    ...
    <div className="container-px mx-auto py-6 md:py-10">
      ...
      <div className="grid md:grid-cols-[1fr_320px] gap-6 md:gap-10">
        <div className="space-y-4">
          {items.map((item) => (
            <div key={`${item.productId}-${item.variantId}`} className="flex gap-4 border-b border-border-soft pb-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2.5">
                  <div className="flex items-center rounded-xl border border-border-soft bg-white">
                    <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity - 1)} className="touch-target !min-h-[40px] !min-w-[40px]" aria-label="Decrease quantity"><Minus size={13} /></button>
                    <span className="w-8 text-center text-sm">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity + 1)} className="touch-target !min-h-[40px] !min-w-[40px]" aria-label="Increase quantity"><Plus size={13} /></button>
                  </div>
                  <button onClick={() => removeItem(item.productId, item.variantId)} aria-label="Remove item" className="touch-target !min-h-[40px] !min-w-[40px] text-ink/70 hover:text-red-500">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
