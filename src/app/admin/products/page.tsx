...
export default async function AdminProductsPage({
  ...
  searchParams: { q?: string; filter?: string; collection?: string; category?: string; sort?: string; page?: string };
}) {
  ...
  // "in stock" means at least one well-stocked variant, "out of stock" means
  // every variant is at 0, "low stock" means some variant is low but not all zero.
  if (filter === "inStock") where.variants = { some: { stock: { gte: 10 } } };
  if (filter === "outOfStock") where.variants = { every: { stock: 0 } };
  if (filter === "lowStock") where.variants = { some: { stock: { gt: 0, lt: 10 } } };
  if (collectionFilter) where.collectionId = collectionFilter;
  ...
  const [
    ...
    prisma.product.findMany({
      ...
      take: PAGE_SIZE,
      include: { collection: { select: { id: true, name: true } }, category: { select: { id: true, name: true } }, variants: true },
    }),
    ...
    prisma.product.count({ where: { variants: { some: { stock: { gt: 0, lt: 10 } } } } }),
  ...
  // of them (e.g. clicking a status tab keeps the current collection/category/sort).
  function buildQuery(overrides: Record<string, string | undefined>) {
    const params = new URLSearchParams();
    const merged = { q, filter, collection: collectionFilter, category: categoryFilter, sort, ...overrides };
  ...
  return (
    <div>
      ...
      {/* Filters + collection/category + sort */}
      <div className="mb-6 flex flex-col gap-3 lg:flex-row">
        ...
        <div className="flex gap-2 lg:ml-auto flex-wrap">
          <Suspense fallback={<div className="rounded-full border border-ink/10 px-4 py-2.5 text-xs bg-white w-28 h-9" />}>
            <SortSelect paramName="collection" options={[{ value: "", label: "All Collections" }, ...collections.map((c) => ({ value: c.id, label: c.name }))]} />
          </Suspense>
          <Suspense fallback={<div className="rounded-full border border-ink/10 px-4 py-2.5 text-xs bg-white w-28 h-9" />}>
      ...
      <ProductsTableClient products={products as any} collections={collections} categories={categories} />
