...
export default async function AdminDashboard() {
  ...
  const [
    ...
    // Stock lives per variant now — these count PRODUCTS with at least one
    // variant in that state, not raw variant rows.
          ...
          {lowStockVariants.length === 0 ? (
            ...
            <div className="space-y-3">
              {lowStockVariants.map((v) => (
                <Link key={v.id} href={`/admin/products/${v.product.id}/edit`} className="flex justify-between text-sm border-b border-ink/5 pb-3 hover:bg-beige/30 -mx-2 px-2 rounded transition-colors">
                  <span className="truncate pr-3">{v.product.name} — {v.color} / {v.size}</span>
                  <span className="text-gold font-medium shrink-0">{v.stock} left</span>
                </Link>
              ))}
            </div>
          )}
