...
export default async function AdminReportsPage({ searchParams }: { searchParams: ReportFilterParams }) {
  ...
  const [report, products, categories, collections] = await Promise.all([
      ...
      {s.orders === 0 ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            <ReportTable title="Collection sales" labelHeader="Collection" rows={report.collections} limit={ON_SCREEN_ROWS} />
          ...
          <p className="text-xs leading-relaxed text-ink/50">
            Product, category and collection totals are line-item values, so they add up to the items subtotal rather than the
