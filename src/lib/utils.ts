...
// Intl's "BDT" currency style renders as the literal ISO code ("BDT 1,500") —
// en-BD has no glyph mapping for the Taka sign in most runtimes. Real
// Bangladeshi retail (and the brand's own currency.symbol) uses "৳1,500"
// instead, which reads as native/premium rather than an invoice line item.
export function formatBDT(amount: number) {
  const grouped = new Intl.NumberFormat("en-BD", { maximumFractionDigits: 0 }).format(amount);
  return `${brand.currency.symbol}${grouped}`;
}
...
export function generateOrderNumber() {
  ...
  return `${brand.shortName}${y}${m}${d}-${rand}`;
}
