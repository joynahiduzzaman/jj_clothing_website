...
export default function FrequentlyBoughtTogether({ items }: { items: FbtProduct[] }) {
  ...
  function addAllToCart() {
    ...
    selected.forEach((item) => {
      const variant = item.variants.find((v) => v.stock > 0);
      if (!variant) return; // nothing in stock to add for this item — skip rather than add an invalid line
      ...
      addItem({
        ...
        slug: item.slug,
        image: images[0] || "",
        price: discountedPrice(variant.priceOverride ?? item.price, item.discountPercent),
        quantity: 1,
        stock: variant.stock,
        color: variant.color,
        size: variant.size,
    ...
    toast.success(`Added ${addedCount} item${addedCount === 1 ? "" : "s"} to bag`);
  }
