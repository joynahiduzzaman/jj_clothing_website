 ...
 * shows only what a buying decision needs (color/size, price, stock, add to
 * cart) and links out for the rest, rather than trying to reproduce the
 * product page.
 *
...
export default function QuickViewModal({
  ...
  const finalPrice = discountedPrice(selectedVariant?.priceOverride ?? product.price, product.discountPercent);
  const outOfStock = !selectedVariant || selectedVariant.stock === 0;
  const lowStock = selectedVariant && !outOfStock && selectedVariant.stock <= 5;
          ...
          <div className="flex min-w-0 flex-1 flex-col p-5">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {sizesForColor.map((v) => (
                  <button
                    ...
                    className={`rounded-lg border px-2.5 py-1 text-[11px] font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-35 ${
            ...
            <p className="mt-2 text-xs text-ink/60">
              {outOfStock ? (
                ...
                <span className="font-semibold text-badge-today-text">Only {selectedVariant?.stock} left</span>
              ) : (
                <span>In stock</span>
              )}
            </p>
