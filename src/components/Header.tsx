...
export default function Header({
  ...
  menuImages?: { collections?: string | null; categories?: string | null };
  /** Read from the catalogue in the layout (see server/nav-menu.ts). Both were
   *  hardcoded lists that went stale: a panel could advertise an entry with no
   *  products and hide a dozen that had them. Only entries with at least one
   *  active product arrive here, so a menu link always lands on a page with
   *  something on it. */
  collectionLinks?: MegaMenuLink[];
      ...
      <header className="sticky top-0 z-50 safe-top">
        ...
        <div className="container-px mx-auto flex items-center justify-between gap-2 sm:gap-6">
          ...
          >
            ...
            <span className="hidden sm:flex flex-col justify-center leading-none">
              <span className="font-display text-[26px] font-semibold text-ink tracking-wide">{brand.name}</span>
              <span className="mt-1.5 text-[10px] uppercase tracking-[0.28em] text-rose-gold-text">{brand.tagline}</span>
            </span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
            <MegaMenu
              ...
              triggerHref="/collections"
              links={collectionLinks}
              image={menuImages?.collections || MENU_IMAGE_FALLBACK}
              caption="Seasonal drops and curated edits."
            ...
            <MegaMenu
              ...
              caption="Shop the full range, by category."
            ...
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="relative text-[13px] font-semibold uppercase tracking-[0.06em] text-ink/80 hover:text-rose-gold-text transition-colors py-2 group"
              >
                {link.label}
        ...
        {menuOpen && (
          <nav className="lg:hidden container-px pb-4 flex flex-col bg-cream divide-y divide-border-soft/70 max-h-[calc(100vh-5rem)] overflow-y-auto animate-fade-up">
            <Link href="/collections" onClick={() => setMenuOpen(false)} className="text-[15px] font-medium text-ink/80 py-3.5">
