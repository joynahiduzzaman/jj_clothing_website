...
const TRUST_STRIP = [
  { icon: ShieldCheck, label: "Quality Checked, Every Piece" },
  { icon: Truck, label: "Fast Delivery Across Bangladesh" },
  { icon: BadgeCheck, label: "Easy 7-Day Returns" },
];
...
export default async function Footer({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  return (
    <footer className="bg-ink text-cream mt-16 md:mt-24">
      {/* Trust strip — the brand's core promises made visible before anything
      ...
      <div className="container-px mx-auto py-12 md:py-20 grid grid-cols-2 md:grid-cols-6 gap-x-8 gap-y-10 md:gap-y-14">
        ...
        <div>
          ...
          <ul className="space-y-2.5 text-sm text-cream/60">
            ...
            <li><Link href="/shop?filter=flashsale" className="link-tap hover:text-cream transition-colors">Sale</Link></li>
            <li><Link href="/collections" className="link-tap hover:text-cream transition-colors">Collections</Link></li>
        ...
        <div className="col-span-2 md:col-span-1">
          ...
          <div className="mt-8">
            {/* link-tap is inline-flex, so the text is an anonymous flex item
                that will not shrink below its longest unbreakable word — a
                long handle can be wider than this column once the footer
                splits at md, which would push the whole document sideways on
                a tablet. `anywhere` is the one overflow-wrap value that
                reduces min-content size in flex layout; `break-word` would
                not have fixed this. */}
            <a
              href={instagramUrl}
              target="_blank"
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {instagramPreview.map((img, i) => (
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Open ${brand.name} on Instagram (photo ${i + 1})`}
