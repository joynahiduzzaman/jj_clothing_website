...
const POPULAR_SEARCHES = ["T-Shirts", "Jeans", "Shirts", "Hoodies", "Outerwear"];
const RECENT_SEARCHES_KEY = "clothing_brand_recent_searches";
const MAX_RECENT = 5;
...
export default function SearchOverlay({ label }: { label: string }) {
  ...
  function quickAddToCart(product: SuggestionProduct, e: React.MouseEvent) {
    ...
    addItem({
      ...
      slug: product.slug,
      image: images[0] || "",
      price: discountedPrice(variant.priceOverride ?? product.price, product.discountPercent),
      quantity: 1,
      stock: variant.stock,
      color: variant.color,
      size: variant.size,
      ...
      >
        <Link href={`/product/${product.slug}`} onClick={() => goToProduct(product)} className="flex items-center gap-4 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            {product.collection?.name && (<p className="text-[11px] uppercase tracking-wide text-olive font-medium">{product.collection.name}</p>)}
            ...
            <div className="flex items-center gap-2.5 mt-1 flex-wrap">
              ...
              <StockBadge stock={product.variants.reduce((s, v) => s + v.stock, 0)} />
            </div>
          </div>
                  ...
                  <div role="listbox" id="search-results-listbox" className="max-h-[65vh] overflow-y-auto">
                    ...
                    {query.trim() && !loading && results.length === 0 && (
                      <div className="p-10 text-center">
                        ...
                        <p className="text-xs text-ink/35 mt-1">Try a different collection, category, or color.</p>
                      </div>
                    )}
