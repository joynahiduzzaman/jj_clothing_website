...
interface TrackedItem { name: string; color: string | null; size: string | null; image: string | null; price: number; quantity: number }
                ...
                <div key={i} className="flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(item.color || item.size) && (
                      <p className="text-xs text-ink/70">{[item.color, item.size].filter(Boolean).join(" / ")}</p>
                    )}
