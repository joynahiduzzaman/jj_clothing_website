...
export interface TaxonomyRow {
  ...
  slug: string;
  image?: string | null;
  /** Collections only — stored in Collection.story. */
...
/**
 * Catalogue taxonomy manager, shared by Categories and Collections.
 *
...
export default function TaxonomyManager({
  ...
  const Label = kind === "categories" ? "Category" : "Collection";
  const imageLabel = kind === "categories" ? "Category image" : "Collection logo";
  ...
  const blank = { id: "", name: "", slug: "", image: "", description: "" };
  ...
  async function remove(row: TaxonomyRow) {
    // Category is a required relation on Product, so this is a hard block —
    // Collection is optional (a product just loses the reference), so deleting
    // one with products attached is allowed, just confirmed more pointedly.
    if (kind === "categories" && row.productCount > 0) {
      return toast.error(
    ...
    const warning = row.productCount > 0 ? ` ${row.productCount} product${row.productCount === 1 ? "" : "s"} will lose this ${label}.` : "";
    if (!confirm(`Delete the ${label} “${row.name}”?${warning} This cannot be undone.`)) return;
    setBusy(row.id);
    try {
  ...
  async function bulkDelete() {
    ...
    // Same distinction as remove() above: categories block on products in use,
    // collections don't (Product.collectionId just clears to null).
    const blocked = kind === "categories" ? chosen.filter((r) => r.productCount > 0) : [];
    const deletable = kind === "categories" ? chosen.filter((r) => r.productCount === 0) : chosen;
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="font-display text-2xl font-semibold sm:text-3xl">{kind === "categories" ? "Categories" : "Collections"}</h1>
          ...
          {visible.map((row) => (
            <li key={row.id} className="card-surface group relative overflow-hidden">
              ...
              <div className="flex border-t border-border-soft">
                <a
                  href={kind === "categories" ? `/shop?category=${row.slug}` : `/collections/${row.slug}`}
                  target="_blank"
                ...
                <button
                  ...
                  aria-label={kind === "categories" && row.productCount > 0 ? `Cannot delete ${row.name} — ${row.productCount} products use it` : `Delete ${row.name}`}
                  className={`flex flex-1 items-center justify-center gap-1.5 border-l border-border-soft py-2.5 text-[11px] font-medium transition-colors ${
                    kind === "categories" && row.productCount > 0 ? "cursor-not-allowed text-ink/25" : "text-red-600 hover:bg-red-50"
                  }`}
                >
          ...
          {visible.map((row) => (
            <li key={row.id} className="card-surface flex items-center gap-2 p-3 sm:gap-4 sm:p-3.5">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <IconButton label={`Delete ${row.name}`} onClick={() => remove(row)} disabled={busy === row.id} danger dimmed={kind === "categories" && row.productCount > 0}>
            ...
            <div className="flex-1 space-y-5 px-5 py-5 sm:px-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={kind === "categories" ? "e.g. T-Shirts" : "e.g. Summer Essentials"}
                  className="field"
                />
              </div>
