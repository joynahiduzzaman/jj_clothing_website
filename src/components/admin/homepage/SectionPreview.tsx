...
export default function SectionPreview({ sectionKey, settings }: { sectionKey: string; settings: Record<string, any> }) {
  ...
  useEffect(() => {
    function onMessage(e: MessageEvent) {
      ...
      if (e.data?.type === "admin-preview-ready") setReady(true);
      else if (e.data?.type === "admin-preview-height") setFrameHeight(Math.max(200, e.data.height));
    }
  ...
  useEffect(() => {
    if (!ready) return;
    iframeRef.current?.contentWindow?.postMessage({ type: "admin-preview-update", sectionKey, settings }, window.location.origin);
