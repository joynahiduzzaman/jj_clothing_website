...
interface NavItem {
  label: string;
  href: string;
  icon: "dashboard" | "products" | "orders" | "coupons" | "affiliates" | "support" | "inventory" | "homepage" | "content" | "categories" | "collections" | "reports";
  badge?: number;
}
  ...
  const sidebarContent = (
    <>
      <div className="flex items-center gap-2.5 border-b border-white/10 px-5 py-[18px]">
        <BrandLogo className="h-9 w-9 ring-1 ring-white/15" />
        <div className="min-w-0">
          <p className="font-display text-lg leading-tight text-cream">{brand.name}</p>
  ...
  return (
    <div className="min-h-screen bg-cream md:grid md:grid-cols-[248px_1fr]">
      ...
      <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-white/10 bg-ink px-4 py-3 text-cream md:hidden">
        ...
        <p className="font-display text-base">{brand.name} Admin</p>
      </div>
