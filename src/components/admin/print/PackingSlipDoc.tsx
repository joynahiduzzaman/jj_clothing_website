    ...
    <div className="bg-white border border-ink/10 rounded-xl2 p-10 shadow-soft print:shadow-none print:border-0 print:p-0">
      <div className="flex items-start justify-between mb-10 pb-6 border-b border-ink/10">
        <div>
          ...
          <p className="text-xs text-ink/70 mt-1">Thank you for your order</p>
        </div>
            ...
            <tr key={item.id} className="border-b border-ink/5">
              <td className="py-3">
                ...
                {(item.color || item.size) && (
                  <span className="block text-xs text-ink/70">{[item.color, item.size].filter(Boolean).join(" / ")}</span>
                )}
              </td>
              <td className="py-3 text-xs text-ink/70">{item.variant?.sku || "—"}</td>
