...
import ProductSearchPicker, { SearchableProduct, SearchableVariant } from "./ProductSearchPicker";
...
export default function ManualOrderForm() {
  ...
  function addProduct(p: SearchableProduct, variant: SearchableVariant) {
    setItems((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          color: variant.color,
          size: variant.size,
          image: parseJsonArray(p.images)[0],
              ...
              {items.map((item) => (
                <div key={item.variantId} className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-ink/70">{item.color} / {item.size} · {formatBDT(item.price)} · {item.stock} in stock</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button type="button" onClick={() => updateQuantity(item.variantId, item.quantity - 1)} className="h-7 w-7 rounded-full border border-ink/10 flex items-center justify-center hover:bg-beige/60">
                      <Minus size={12} />
                    </button>
                  ...
                  <button type="button" onClick={() => removeItem(item.variantId)} aria-label={`Remove ${item.name}`} className="text-ink/30 hover:text-red-500 shrink-0">
                    <Trash2 size={15} />
                  </button>
                </div>
              ))}
