...
export default function ProductsTableClient({
  ...
  const [bulkTarget, setBulkTarget] = useState<"changeCategory" | "changeCollection" | null>(null);
  ...
  async function runBulk(action: "delete" | "activate" | "draft" | "changeCategory" | "changeCollection", value?: string) {
    ...
    <div>
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-4 bg-ink text-white rounded-xl px-4 py-3">
          ...
          <button disabled={busy} onClick={() => setBulkTarget("changeCollection")} className="text-xs rounded-full bg-white/10 hover:bg-white/20 px-3 py-1.5 transition-colors">
            Change Collection
          </button>
      ...
      {bulkTarget && (
        <div className="flex items-center gap-2 mb-4 bg-white rounded-xl px-4 py-3 shadow-soft">
          <span className="text-xs text-ink/70 shrink-0">{bulkTarget === "changeCategory" ? "Move to category:" : "Move to collection:"}</span>
          <select
          ...
          >
            ...
            {(bulkTarget === "changeCategory" ? categories : collections).map((o) => (
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3 max-w-xs">
                      ...
                      <div className="min-w-0">
                        ...
                        {firstSku && <p className="text-[11px] text-ink/35 font-mono">{firstSku}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="p-4">{p.collection ? <CollectionBadge name={p.collection.name} /> : <span className="text-ink/30 text-xs">—</span>}</td>
                  <td className="p-4"><CategoryBadge name={p.category.name} /></td>
                  <td className="p-4 font-medium tabular-nums">{formatBDT(p.price)}</td>
                  <td className="p-4 w-32"><StockBadge stock={totalStock} /></td>
                  <td className="p-4"><ProductStatusBadge stock={totalStock} status={p.status} /></td>
                  <td className="p-4">
                    <ProductActionsMenu
                      slug={p.slug}
