...
export function welcomeEmail(name: string) {
  return emailShell(
    ...
    `${heading(`Welcome, ${name}`)}
     ${p(`Thanks for joining ${brandConfig.name} — ${brandConfig.description}`)}
     ${button("Start Shopping", `${siteUrl}/shop`)}
     ${divider()}
     ${p(`<strong style="color:${ink};">Cash on Delivery</strong> across Bangladesh &middot; Dhaka ৳70, outside Dhaka ৳130 &middot; delivered in 1&ndash;5 business days.`, "font-size:13.5px;margin:0;")}`,
    `Welcome to ${brandConfig.name} — ${brandConfig.tagline}`
  );
}
...
export function verificationEmail(name: string, verifyUrl: string) {
  return emailShell(
    ...
    `${heading("Verify your email")}
     ${p(`Hi ${escapeHtml(name)}, please confirm this is your email address to activate your ${brandConfig.name} account.`)}
...
export function passwordResetEmail(name: string, resetUrl: string) {
  return emailShell(
    ...
    `${heading("Reset your password")}
     ${p(`Hi ${escapeHtml(name)}, we received a request to reset the password on your ${brandConfig.name} account.`)}
     ${button("Reset Password", resetUrl)}
     ${p(`<span style="font-size:12.5px;color:#8A8079;">Button not working? Paste this into your browser:</span><br /><a href="${resetUrl}" style="font-size:12px;color:${accentColor};word-break:break-all;">${escapeHtml(resetUrl)}</a>`, "margin-top:18px;")}
    ...
    `Reset your ${brandConfig.name} password. Link expires in 1 hour.`
  );
}
...
export function orderConfirmationEmail(params: {
  ...
  const rows = params.items
    .map(
      (i) =>
        `<tr>
          <td style="padding:11px 0;font-size:14px;color:${ink};border-bottom:1px solid #F1EAE3;">
            ${escapeHtml(i.name)}
            <span style="color:#8A8079;">&times; ${i.quantity}</span>
          </td>
          <td style="padding:11px 0;font-size:14px;color:${ink};text-align:right;white-space:nowrap;border-bottom:1px solid #F1EAE3;">
  ...
  const totalRow = (label: string, value: string, strong = false, color = muted) =>
    `<tr>
      <td style="padding:5px 0;font-size:${strong ? "15px" : "13.5px"};color:${strong ? ink : color};${strong ? "font-weight:700;padding-top:10px;" : ""}">${label}</td>
      <td style="padding:5px 0;font-size:${strong ? "15px" : "13.5px"};color:${strong ? ink : color};text-align:right;white-space:nowrap;${strong ? "font-weight:700;padding-top:10px;" : ""}">${value}</td>
    </tr>`;

  return emailShell(
    `Order Confirmed — ${params.orderNumber}`,
    `${heading(`Thank you, ${params.customerName}`)}
     ${p(`Your order <strong style="color:${ink};">${escapeHtml(params.orderNumber)}</strong> is confirmed and being prepared.`)}
     ...
     <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:14px;">
       ...
       ${params.discount > 0 ? totalRow("Discount", `-${formatBDT(params.discount)}`, false, accentColor) : ""}
       ${totalRow("Shipping", formatBDT(params.shippingFee))}
       ${totalRow("Total", formatBDT(params.total), true)}
     </table>

     ${divider()}
     ${p(`Payment method: <strong style="color:${ink};">${escapeHtml(params.paymentMethod)}</strong>`, "font-size:13.5px;margin:0;")}
...
export function orderStatusEmail(params: {
  ...
  const trackingLine =
    params.courier || params.trackingNumber
      ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:16px 0 0;background:#F7F4EF;border-radius:10px;">
           <tr><td style="padding:14px 16px;font-size:13.5px;color:${muted};">
             ${params.courier ? `Courier: <strong style="color:${ink};">${escapeHtml(params.courier)}</strong>` : ""}
             ${params.courier && params.trackingNumber ? " &middot; " : ""}
             ${params.trackingNumber ? `Tracking #: <strong style="color:${ink};">${escapeHtml(params.trackingNumber)}</strong>` : ""}
           </td></tr>
         </table>`
      : "";
  ...
  return emailShell(
    ...
    `${heading(`Hi ${params.customerName}`)}
     ${p(`Your order <strong style="color:${ink};">${escapeHtml(params.orderNumber)}</strong> ${escapeHtml(params.message)}.`)}
     ${trackingLine}
...
export function newsletterWelcomeEmail() {
  return emailShell(
    ...
    `${heading("You're on the List")}
     ${p(`You'll now get early access to new arrivals, restocks, and sales from ${brandConfig.name}.`)}
     ${button("Shop Now", `${siteUrl}/shop`)}`,
    "Early access to new arrivals, restocks and sales."
  );
}
...
export function abandonedCartEmail(name: string, items: { name: string; image?: string }[], resumeUrl: string) {
  const rows = items
    ...
    .map(
      (i) =>
        `<tr><td style="padding:9px 0;font-size:14px;color:${ink};border-bottom:1px solid #F1EAE3;">${escapeHtml(i.name)}</td></tr>`
    )
    .join("");
  ...
  return emailShell(
    ...
    "Your bag is waiting — finish your order in a couple of taps."
  );
}
...
export function contactFormEmail(params: { name: string; email: string; subject: string; message: string }) {
  return emailShell(
    ...
    `${heading("New message from the Contact page")}
     <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:13.5px;color:${ink};margin-bottom:18px;">
     ...
     <div style="font-size:14.5px;line-height:1.7;color:${ink};white-space:pre-wrap;border-left:3px solid ${accentColor};padding-left:14px;">${escapeHtml(params.message)}</div>
...
export function newOrderAdminEmail(params: {
  ...
  const detail = (label: string, value?: string | null) =>
    value
      ? `<tr>
          ...
          <td style="padding:4px 12px;font-size:13.5px;color:${ink};">${escapeHtml(value)}</td>
        </tr>`
      : "";
  ...
  return emailShell(
    ...
    `${heading(`New order · ${escapeHtml(params.orderNumber)}`)}
     ${p(`<strong style="color:${ink};">${escapeHtml(params.customerName)}</strong> placed an order for <strong style="color:${ink};">${formatBDT(params.total)}</strong> — ${unitCount} item${unitCount === 1 ? "" : "s"}, paid by ${escapeHtml(params.paymentMethod)}.`)}
