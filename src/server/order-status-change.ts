...
export async function applyOrderStatusChange(
  ...
  if (!wasRestored && nowRestored) {
    for (const item of existing.items) {
      if (!item.variantId) {
        console.warn(`Order ${existing.orderNumber}: item ${item.id} has no variantId, skipping stock restore`);
        continue;
      }
      await adjustStock(item.variantId, item.quantity, `Order ${existing.orderNumber} ${toStatus.toLowerCase()} — stock restored`, admin.name);
    }
  } else if (wasRestored && !nowRestored) {
    for (const item of existing.items) {
      if (!item.variantId) {
        console.warn(`Order ${existing.orderNumber}: item ${item.id} has no variantId, skipping stock re-reservation`);
        continue;
      }
      await adjustStock(item.variantId, -item.quantity, `Order ${existing.orderNumber} reactivated — stock re-reserved`, admin.name);
    }
  }
