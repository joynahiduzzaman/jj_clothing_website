...
export async function verifyOrderItems(
  ...
  const verifiedItems: VerifiedLine[] = items.map((item) => {
    ...
    if (!dbVariant || dbVariant.productId !== item.productId) {
      throw new OrderValidationError(`Product variant not found (${item.variantId})`);
    }
    const dbProduct = dbVariant.product;
    if (dbProduct.status === "DRAFT") {
      throw new OrderValidationError(`${dbProduct.name} is no longer available for purchase`);
    }
    const label = `${dbProduct.name} (${dbVariant.color}/${dbVariant.size})`;
    if (dbVariant.stock < item.quantity) {
      if (opts.allowInsufficientStock) {
        stockWarnings.push(`${label}: only ${dbVariant.stock} in stock, order requests ${item.quantity}`);
      } else {
        throw new OrderValidationError(`${label} only has ${dbVariant.stock} left in stock`);
      }
    }
    return {
      ...
      price: discountedPrice(dbVariant.priceOverride ?? dbProduct.price, dbProduct.discountPercent),
...
export async function resolveCoupon(code: string | undefined, subtotal: number): Promise<{ discount: number; couponCode?: string }> {
  ...
  if (coupon && coupon.active && notExpired && underLimit && subtotal >= coupon.minSpend) {
    ...
    // A FIXED coupon can be configured with a value larger than a given cart's
    // subtotal. Without this clamp, createOrderRecord's
    // `subtotal - discount + shippingFee` computes as one expression before
    // its own floor at 0 — so an oversized discount silently zeroed out
    // shipping too, rather than just capping the discount at "this order is
    // free." Capping here keeps the discount honest on its own line item.
...
export async function finalizeOrderEffects(
  ...
  for (const item of verifiedItems) {
    await adjustStock(item.variantId, -item.quantity, `Order ${order.orderNumber} placed`);
  }
  ...
  if (recipientEmail) {
    ...
    const result = await sendOrderConfirmationEmail(recipientEmail, {
      ...
      customerName,
      items: verifiedItems.map((i) => ({ name: i.color || i.size ? `${i.name} (${[i.color, i.size].filter(Boolean).join(" / ")})` : i.name, quantity: i.quantity, price: i.price })),
