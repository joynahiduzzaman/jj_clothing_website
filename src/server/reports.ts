...
export function parseReportFilters(sp: ReportFilterParams): ParsedReportFilters {
  ...
  const collectionId = sp.collectionId?.trim() || "";
  return {
    ...base,
    productId,
    categoryId,
    collectionId,
    active: base.active || Boolean(productId || categoryId || collectionId),
  };
}
 ...
 *
 * The product/category/collection filters are a `some` on the order's items:
 * an order counts if it contains at least one matching line. That means an
 * order's *total* can't be attributed to one product — so the summary reports
 * order counts and totals for orders containing that product, while the
 * per-product, per-category and per-collection breakdowns below work from
 * line items, which do attribute cleanly. Two different questions, answered
 * separately rather than one number pretending to answer both.
 */
export function reportWhere(f: ParsedReportFilters): Record<string, any> {
  ...
  if (f.collectionId) itemFilter.product = { ...(itemFilter.product || {}), collectionId: f.collectionId };
 ...
 *  product/category/collection — the basis for every breakdown. */
function itemWhere(f: ParsedReportFilters): Record<string, any> {
  ...
  if (f.categoryId || f.collectionId) {
    where.product = {};
    if (f.categoryId) where.product.categoryId = f.categoryId;
    if (f.collectionId) where.product.collectionId = f.collectionId;
  }
  return where;
}
...
/** Product, category and collection breakdowns all group the same line items
 *  by a different key, so they share one query and one shape. */
async function breakdown(f: ParsedReportFilters, by: "product" | "category" | "collection"): Promise<BreakdownRow[]> {
  ...
  for (const it of items) {
    ...
    if (by === "product") {
      ...
      sublabel = it.product?.collection?.name;
    } else if (by === "category") {
      key = it.product?.category?.id || "none";
      label = it.product?.category?.name || "(uncategorised)";
    } else {
      key = it.product?.collection?.id || "none";
      label = it.product?.collection?.name || "(no collection)";
    }
...
export const getCollectionSales = (f: ParsedReportFilters) => breakdown(f, "collection");
...
export async function getFullReport(f: ParsedReportFilters): Promise<FullReport> {
  const [summary, byDay, products, categories, collections, payments, sources] = await Promise.all([
    getSalesSummary(f),
    getSalesByDay(f),
    getProductSales(f),
    getCategorySales(f),
    getCollectionSales(f),
    getPaymentMethodSales(f),
    getSourceSales(f),
  ]);
  return { summary, byDay, products, categories, collections, payments, sources };
}
